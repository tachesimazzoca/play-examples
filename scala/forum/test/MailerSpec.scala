...
class MailerSpec extends Specification {
  ...
  "SignupMailer" should {
    "render views.txt._mail.signup" in new WithApplication(FakeApplication()) {
  ...
  "loadSettings" should {
    "load settings with Play configuration" in new WithApplication(
    ...
    "load some part of settings and merge defaults" in new WithApplication(
        FakeApplication(additionalConfiguration = ConfigFactory.parseString("""
          |mailer.TestMailer: {
